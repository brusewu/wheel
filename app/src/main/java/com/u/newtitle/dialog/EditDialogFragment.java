...
import com.u.newtitle.util.AnimationLoader;
...
public class EditDialogFragment extends DialogFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //设置样式
        setStyle(android.support.v4.app.DialogFragment.STYLE_NORMAL, R.style.ic_dialog_style);
        //加载动画
    ...
    public void onClicked(View view) {
        ...
        switch (view.getId()){
            case R.id.tv_positive:
                if (mListener != null){
                    HashSet<String> inputData = new HashSet<>();
                    if (mIsEditText1Enable) {
                        inputData.add(editText1.getText().toString());
                        editText1.getText().clear();
                    }
                    if (mIsEditText2Enable) {
                        inputData.add(editText2.getText().toString());
                        editText2.getText().clear();
                    }
                    if (mIsEditText3Enable) {
                        inputData.add(editText3.getText().toString());
                        editText3.getText().clear();
                    }
                    //隐藏软盘
                    UIUtils.hideSoftKeyboard(getActivity());
                    mListener.onConfirm(inputData);
                }
                break;
    ...
    private void setBtnBackground(final TextView textView, boolean isPosition) {
        if (isPosition){
            ...
            textView.setBackground(getContext().getResources().getDrawable(getSelBtn(getDialogType())));
        }else {
            textView.setTextColor(createColorStateList(getContext().getResources().getColor(getColorResId(DIALOG_TYPE_DEFAULT)),
                    getContext().getResources().getColor(R.color.color_dialog_gray)));
            textView.setBackground(getContext().getResources().getDrawable(getSelBtn(DIALOG_TYPE_DEFAULT)));
        }
    }
    ...
    public interface OnDialogListener {
        ...
        void onConfirm(HashSet<String> inputData);
    }
}
